...
void add_element_for_y_exp(wchar_t first_of_rhyme_non_tone,wchar_t *rhyme,wchar_t *phone_rhyme);
...
void read_file_tricky_phone();
wchar_t * split(wchar_t *buff, wchar_t symbol);
void remove_q_from_initial_consonants();
void convert_result_to_tricky_phone(char *source, char *destiny,int max_size,int max_len,wchar_t tricky_source_other_form[max_size][max_len],int max_len1,wchar_t tricky_destiny_other_form[max_size][max_len1]);
void convert_tricky_to_other_form(int max_size,int max_len,wchar_t tricky_other_form[max_size][max_len],int max_len1,wchar_t tricky[max_size][max_len1],wchar_t* char_bound);
...
wchar_t tricky_source_non_festival[TRICKY_MAX_SIZE][TRICKY_SOURCE_OTHER_FORM_MAX_LENGH];
wchar_t tricky_destiny[TRICKY_MAX_SIZE][TRICKY_DESTINY_MAX_LENGH];
wchar_t tricky_destiny_non_festival[TRICKY_MAX_SIZE][TRICKY_DESTINY_OTHER_FORM_MAX_LENGH];
wchar_t tricky_destiny_festival[TRICKY_MAX_SIZE][TRICKY_DESTINY_OTHER_FORM_MAX_LENGH];
wchar_t tricky_source_festival[TRICKY_MAX_SIZE][TRICKY_SOURCE_OTHER_FORM_MAX_LENGH];
...
void convert_tricky_to_other_form(int max_size,int max_len,wchar_t tricky_other_form[max_size][max_len],int max_len1,wchar_t tricky[max_size][max_len1],wchar_t* char_bound){
    ...
    for(i=0;i<tricky_size;i++){
        wcscpy(tricky_other_form[i],tricky[i]);
        wcscat(tricky_other_form[i],char_bound);
    }
}

//luu y trong file tricky_phone chu p phai sau chu o vi o = 7- ma p lai la vi17-
void convert_result_to_tricky_phone(char *source, char *destiny,int max_size,int max_len,wchar_t tricky_source_other_form[max_size][max_len],int max_len1,wchar_t tricky_destiny_other_form[max_size][max_len1]){
    FILE *source_file = fopen(source,"r"), *destiny_file=fopen(destiny,"w");
    wchar_t *syllable=(wchar_t*)malloc(SYLLABLE_MAX_LENGH*sizeof(wchar_t)),phone_syllable_tricky[PHONE_SYLLABEL_MAX_LENGH+15];
    wchar_t tricky_be_replace[TRICKY_SOURCE_MAX_LENGH+2];
    int i;
    while(fgetws(syllable,256,source_file)!=NULL){
        syllable[wcslen(syllable)-1]=L'\0';
        wcscpy(phone_syllable_tricky,split(syllable,L' '));
        for(i=0;i<tricky_size;i++){
...
void remove_q_from_initial_consonants(){
	...
	while(i<consonant_size){
        ...
        wcscpy(phone_consonants[i],phone_consonants[i+1]);
		i++;
	}
	consonant_size--;
}
...
void read_file(FILE *f, int max_size, int max_len1,int max_len2,wchar_t string1[max_size][max_len1], wchar_t string2[max_size][max_len2],int *count){
		...
		while(fgetws(string,256,f)!=NULL){
    		...
    		wcscpy(string2[*count],split(string,L' '));
    		wcscpy(string1[*count],string);
    		(*count)++;
		}
}
	...
	for(i=0;i<rhyme_size;i++){
		if(wcscmp(rhymes[i],L"iêc")==0||
		   ...
		   if(is_end_ptk(temp_rhyme))
			   insert_tone_and_join_end_ptk(find_index_to_insert_tone,add_element_for_y_exp,temp_rhyme,phone_rhymes[i]);
		   else{
			   wcscat(phone_rhymes[i],L"-A1");
			   add_element_for_y_exp(temp_rhyme[0],temp_rhyme,phone_rhymes[i]);
			   insert_tone_and_join_non_end_ptk(find_index_to_insert_tone,add_element_for_y_exp,temp_rhyme,phone_rhymes[i]);
		   }
		}
	}
}
...
void replace_one_char_with_string(wchar_t *string, wchar_t *result_string, wchar_t symbol_be_replace, wchar_t *string_replace){
    ...
    while((split_result = split(temp_string,symbol_be_replace))!=NULL){
        wcscat(result_string,temp_string);
        wcscat(result_string,string_replace);
        wcscpy(temp_string,split_result);
    }
...
wchar_t *replace_all_string_with_string(wchar_t *str, wchar_t *orig, wchar_t *rep)
{
    wchar_t buffer[256], *string=(wchar_t*)malloc(256*sizeof(wchar_t)),*result=(wchar_t*)malloc(256*sizeof(wchar_t)),*p;
    int len_orig=wcslen(orig), len_rep=wcslen(rep);
    wcscpy(string,str);
    wcscpy(result,L"");
    // printf("string %ls orig %ls rep %ls\n",string,orig,rep);

    while((p = wcsstr(string, orig))){
        wmemcpy(buffer, string, p-string);
        buffer[p-string] = L'\0';
        wcscat(buffer,rep);
...
void add_element(wchar_t first_of_rhyme_non_tone,wchar_t *rhyme,wchar_t *phone_rhyme){
	...
	if(first_of_rhyme_non_tone!=L'i'||(len<2||rhyme[3]==L'h')){ // xem lai doan nay
